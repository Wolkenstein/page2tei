...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xd:doc scope="stylesheet">
    <xd:desc>
      ...
      <xd:p/>
      <xd:p><xd:b>Contributor</xd:b> Peter Hinkelmanns, github:@Wolkenstein</xd:p>
      <xd:p>Mittelhochdeutsche Begriffsdatenbank, University of Salzburg</xd:p>
      <xd:p>Added support for multiple custom tags from transkribus</xd:p>
    </xd:desc>
  </xd:doc>
  ...
  <xsl:variable name="resultTransformation1">
    <xsl:apply-templates mode="transformation1"/>
  </xsl:variable>

  <xsl:template match="/mets:mets" mode="transformation1">
    <TEI>
      <teiHeader>
        <fileDesc>
          <titleStmt>
          ...
          <seriesStmt>
            <xsl:apply-templates
              select="mets:amdSec//trpDocMetadata/collectionList/colList[1]/colName"/>
          </seriesStmt>
          <sourceDesc>
    ...
    <xsl:choose>
      ...
      <xsl:when test="@type = 'person'">
        ...
        <xsl:element name="{$elName}">
          ...
          <!--added support for wikidata-attribute-->
          <xsl:if test="$custom('wikidata')">
            <xsl:attribute name="ref" select="replace($custom('wikidata'), '\\u003a', ':')"/>
          </xsl:if>
        ...
        <xsl:element name="w">
          <xsl:if test="$custom('fwb') != '' or $custom('wbnetz') != ''">
            <xsl:attribute name="lemmaRef"
              select="normalize-space(replace($custom('fwb'), '\\u003a', ':') || ' ' || replace($custom('wbnetz'), '\\u003a', ':'))"
            />
          </xsl:if>
          <xsl:call-template name="elem">
            <xsl:with-param name="elem" select="$elem"/>
      ...
      <xsl:when test="@type = 'comment'">
        <note>
          <xsl:value-of select="replace($custom('comment'), '\\u0020', ' ')"/>
        </note>
        <xsl:call-template name="elem">
          <xsl:with-param name="elem" select="$elem"/>
        </xsl:call-template>
  ...
  <!-- Start of second transformation -->

  <xsl:template match="/">
    <xsl:apply-templates select="$resultTransformation1" mode="transformation2"/>
  </xsl:template>
  ...
  <xsl:template match="tei:rs[@continued = 'true']" mode="transformation2">
    <xsl:variable name="part1" select="."/>
    <xsl:variable name="part2" select="./following-sibling::tei:rs[@continued = 'true'][1]"/>
    <xsl:variable name="nodeList"
      select="$part1/following-sibling::node() intersect $part2/preceding-sibling::node()"/>
    <xsl:if test="exists($nodeList)">
      <xsl:element name="rs">
        <xsl:for-each select="$part1/@*">
          <xsl:if test="name() != 'continued'">
            <xsl:copy>.</xsl:copy>
          </xsl:if>
        </xsl:for-each>
        <xsl:copy-of select="$part1/node()"/>
